...
- Основная функциональность:
    - Создание сниппета с указанным временем жизни, политикой доступа(public, protected, private) и паролем, если
      сниппет
...
- [Технологический стек](#-технологический-стэк)
...
├── 📂test/                            # файлы интеграционных тестов
...
#### Примечание: подробную информацию по доступным командам `make` можно получить через `make help`
...
1. Клонировать репозиторий: `git clone https://github.com/StewardMcCormick/go-paste-bin.git`
2. Перейти в папку с проектом: `cd ./go-paste-bin`
3. Скопировать `.env.example` в `.env` и заполнить значениями для своих PostgreSQL и Redis (запустить их, если это не
   сделано)
...
1. Клонировать репозиторий: `git clone https://github.com/StewardMcCormick/go-paste-bin.git`
2. Перейти в папку с проектом: `cd ./go-paste-bin`
3. Скопировать `.env.docker-single.example` в `.env.docker-single` и заполнить значениями для своих PostgreSQL и Redis(
   запустить их, если это не сделано)
...
- `400 ErrBadRequest`: при невалидном теле запроса (неправильный JSON или ошибка валидации - в этом случае содержит
  информацию о неправильно заполненных полях)
...
- `401 ErrUnauthorized`: если пользователь прислал неверный пароль/API-ключ

### 💬 Работа со сниппетами:

#### Примечание: во всех запросах ниже сервер может вернуть `401 ErrUnauthorized` при невалидном API-ключе
...
- `403 ErrForbidden`: если пользователь запрашивает `private` сниппет, который ему не принадлежит
...
- `200 OK`: с информацией о сниппете
- `401 ErrUnauthorized`: если пароль неверный
- `404 Not Found`: если сниппет не был найден
- `400 ErrBadRequest`: при невалидном теле запроса (неправильный JSON или ошибка валидации - в этом случае содержит
  информацию о неправильно заполненных полях)
...
#### Примечание: все поля в этом запросе опциональны, кроме случая, когда `privacy` устанавливается в
`protected` - тогда поле `password` обязательно
...
- **Интеграционные тесты** покрывают взаимодействие репозиторного слоя с внещними БД (поднимаются тестовые инстансы с
  помощью testcontainers)
