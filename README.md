...
#### Примечание: 
во всех запросах ниже сервер может вернуть `401 Unauthorized` при невалидном API-ключе
...
```
#### Примечание: 
все поля в этом запросе опциональны, кроме случая, когда `privacy` устанавливается в `protected` - тогда поле `password` обязательно
